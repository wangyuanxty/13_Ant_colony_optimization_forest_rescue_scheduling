...
A2C 策略以 121 次灭火（+23%）和更低火灾损失（−4%）超越了贪心最近 baseline。主要归因于任务链周转速度的提升：A2C 策略通过共享的空间嵌入隐式协调无人机—直升机—地面队的空间分布，使"侦察→运输→灭火"全链路更紧凑，同一网格可在 episode 内被多次灭火（燃烧→运输→灭火→重燃→再灭火），更高的周转率直接体现为更高的灭火计数。
