...
A2C 策略以 121 次灭火（+23%）和更低火灾损失（−4%）超越了贪心最近 baseline。主要原因：多智能体并发时存在目标冲突（例如两架直升机同时"最近"同一火点，贪心策略先决策者抢占、后决策者被迫绕远），A2C 通过共享空间嵌入隐式学习到更均衡的任务分配。此外，A2C 策略在无人机—直升机—地面队的任务链周转速度上优于 Nearest，同一网格可在 episode 内被多次灭火（燃烧→运输→灭火→重燃→再灭火），更快的周转直接体现为更高的灭火计数。
