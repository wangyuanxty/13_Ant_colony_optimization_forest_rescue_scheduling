...
A2C 策略以 121 次灭火（+23%）和更低火灾损失（−4%）超越了贪心最近 baseline。需注意 Nearest baseline 的实现存在一个协同缺陷：各智能体独立选择最近目标，无 `taken` 标记机制，多个同类型智能体可能被派往同一火点，造成冗余飞行。A2C 策略通过 mask 排除已占用目标，天然避免了此问题。该优势主要来自架构层面的冲突避免，而非策略学习本身。
